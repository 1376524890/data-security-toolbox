...
BUILTIN_RULES: dict[str, dict[str, Any]] = {
    "PROTO_DNS_TUNNEL_001": {
        ...
        "title": "DNS 隧道 / 异常编码域名",
        "severity": "High",
        "condition": "同一源主机+注册域在时间窗口内：编码左标签数 >= min_encoded_labels 且查询数 >= min_queries（孤立高熵/长域名只记候选）",
    ...
    "PROTO_HTTP_UPLOAD_001": {
        ...
        "severity": "High",
        "condition": "上传方法 且 Content-Type 为 multipart/octet-stream 且 URI/正文含动态脚本扩展名",
    ...
    "NETWORK_PORT_SCAN": {
        ...
        "title": "端口扫描",
        "severity": "High",
        "condition": "同一源地址在 port_scan_window_seconds 窗口内访问的不同目标端口数 >= port_scan_ports_threshold",
    ...
    "NET_C2_BEACON_001": {
        ...
        "title": "C2 周期性心跳",
        "severity": "High",
        "condition": "同一五元组报文数 >= min_packets 且 观察时长 >= min_duration_seconds 且 平均间隔 >= min_interval_seconds 且 间隔标准差/均值 < max_interval_cv",
