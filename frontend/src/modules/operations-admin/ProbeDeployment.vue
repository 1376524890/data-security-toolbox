...
import type { CreateDeploymentPayload, ProbeDeployment, ProbeDeploymentDetail, PreflightResult } from '../../types/probeDeployment'
  ...
  <div>
    <div class="toolbar">
      ...
      <el-select v-model="filters.status" placeholder="状态" clearable style="width: 160px; margin-left: 8px">
        <el-option v-for="s in ['CREATED', 'CONNECTING', 'PREFLIGHT', 'UPLOADING', 'INSTALLING', 'WAIT_CALLBACK', 'REGISTERED', 'ONLINE', 'FAILED']" :key="s" :label="s" :value="s" />
      </el-select>
    ...
    <StateBox :loading="loading" :error="error" :empty="!items.length" @retry="load">
      <el-table :data="items" size="small" @row-click="open">
        ...
        <el-table-column label="操作" width="120"><template #default="{ row }"><el-button size="small" type="primary" @click.stop="open(row)">详情</el-button><el-button v-if="row.status === 'FAILED'" size="small" type="warning" @click.stop="retry(row)">重试</el-button></template></el-table-column>
      </el-table>
    ...
    <DetailDrawer v-model="drawer" title="部署详情" width="60%">
      <template v-if="detail">
        ...
        <div v-if="detail.preflight_result" class="sec-title" style="margin-top: 14px">预检结果</div>
        <el-table v-if="detail.preflight_result" :data="(detail.preflight_result.checks as any[])" size="small">
