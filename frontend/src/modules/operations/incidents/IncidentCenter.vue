...
import type { AttackStage } from '../../../types/incident'
import { useIncidentCenter } from './composables/useIncidentCenter'

// The list, the detail, the status transitions and the correlation tool live
// in the composable; this view only keeps the filter fields and the
// attack-stage labels.
const {
  loading, error, items, total, selected, detail, detailLoading, filters, activeStages,
  findings, correlateOpen, correlateRunning, correlateFindings, correlateWindow,
  correlateResult, load, open, changeStatus, reset, runCorrelate,
} = useIncidentCenter()
