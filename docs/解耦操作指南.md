...
| P1 | `services/dlp_service.py` 460 行把策略默认值、自身流量判定、抓包重组、HTTP 对象解析与检测编排放在一起；`rule_library` 又 `from app.services.dlp_service import masked`，与 DLP 侧 import 规则库常量形成互导 | 改 DLP 策略要先读抓包代码；规则库与网络 DLP 无法独立演进 | I |
| P1 | `ruleset_service.import_working_rules` 自己读 `dlp_rules/*.json` 做第二份映射（漏 `validator`），`api/libraries.set_dlp_rule` 与 `api/rule_presenter` 各自 glob 规则库目录 | 同一条规则在平台与探针口径漂移；规则库布局改动要改多处 | I |
...
### C. 按业务域逐个拆路由（PCAP、文件、平台资产、事件/情报、告警、任务/审计/报表、检测/引擎、看板/流量视图、探针、集成与离线导入、规则域、v1 剩余零散入口 auth/health/scan/test 2026-09-20 已完成）
...
拆分前后 OpenAPI 排序后字节一致。
落地位置（探针域）：探针注册/心跳/列表/删除/分析/扫描/指标与加密画像拆到 `api/probes.py`（9 条路径：
`/probes/register`、`/probes/{probe_id}/heartbeat`、`/probes`、`/probes/{probe_id}`、
`/probes/{probe_id}/analyze`、`/probes/{probe_id}/tasks`、`/probes/{probe_id}/scan`、
`/probes/{probe_id}/metrics`、`/crypto/probe-profile`）；探针鉴权仍走 `core/security.py` 与
`api/dependencies.py`，登记入册仍在 `deployment/enrollment.py`，删除与远端卸载仍在
`services/probe_service.py`、`deployment/removal.py`，行序列化复用 `api/probe_presenter.py::serialize_probe`、
`api/task_presenter.py::serialize_task`；本域在 v1 内用的私有派发 `_dispatch` 下沉为共享
`api/dependencies.py::dispatch_task`（docstring 原样保留），v1 不再保留副本，另修掉随域移动的两处
`raise ... from exc`（B904）。边界由 `tests/test_probe_boundaries.py`（9 项）锁定，拆分前后 OpenAPI
排序后字节一致，且 158 个操作的「方法 + 路径」首个匹配函数与拆分前逐条相同；探针下发/回收、规则下发、
扫描与采集上报仍分属 `deployments.py`、`rulesets.py`、`extensions.py`、`data_collection.py`，不并入本域。
落地位置（集成与离线导入域）：适配器目录与手动执行、以及整个 `/offline` 面收拢到
`api/integrations.py`（11 条路径），其中 `/offline/*` 原先分散在 `api/v1.py`（资源/CVE 列表、上传）与
`api/libraries.py`（手工 CVE、Grype 更新/导入/任务），本批一并归位，`libraries.py` 只留 dlp 规则、
规则源与规则导入；适配器元数据/执行、离线包解析、Grype 库、告警与事件聚合仍在 `app.integrations`、
`services/grype_library.py`、`services/alert_service.py`、`incident_engine`。四条从 libraries 迁来的路由
保留原 `rule-libraries` tag，拆分前后 OpenAPI 排序后**字节一致**；worker 能力与规则清单读取
（`read_worker_capabilities`、`merge_capability`、`engine_rule_counts`）下沉到 `api/runtime_status.py`，
由 `/health` 与本域共用。边界由 `tests/test_integration_offline_boundaries.py`（8 项）锁定。
落地位置（规则域）：检测引擎规则面收拢到 `api/rules.py`（5 条路径：规则列表、规则内容、
规则来源、在线同步、手工导入），其中 `/rule-sources`、`POST /rules/sync`、`POST /rules` 原在
`api/libraries.py`，本批归位后 `libraries.py` 只保留敏感数据（DLP）规则族 `/dlp/rules*`（217 → 101 行）；
三条迁出路由保留原 `rule-libraries` tag，拆分前后 OpenAPI 排序后**字节一致**。规则枚举仍只有
`api/rule_presenter.py::rule_file_entries` 一份，来源/拉取/校验仍分别走 `app.rules.catalog`、
`app.rules.sync`、`app.integrations.offline_manager`；边界由 `tests/test_rule_boundaries.py`（8 项）锁定。
落地位置（v1 剩余零散入口）：`api/auth.py`（`POST /auth/login|logout`、`GET /auth/me`）、
`api/health.py`（`GET /health`）、`api/network_scan.py`（`POST /scan`、`GET /scan/{task_id}`）、
`api/test_data.py`（`POST /test/import|clear`、`GET /test/status`，opt-in 守卫
`_require_test_data_import` 随域下沉）。四域都只 include 一次，`v1.py` 自此不再声明任何路径
（288 → 79 行，只剩兼容重导出与聚合）；共享边界（`core/security.py`、`api/runtime_status.py`、
`services/test_service.py`、`services/scan_service.py`、`api/dependencies.dispatch_task`）不复制。
边界由 `tests/test_{auth,health,network_scan,test_data}_boundaries.py`（29 项）锁定，
其中 `test_auth_boundaries.py` 额外要求 `v1.py` 的路由声明为空集，防止零散入口再回流。
其余建议顺序：前端页面（类型中心/对象详情/采集任务页；PCAP 工作台状态已抽完）→ 模型包 → 探针模块化。
下列条目保留为当时的实施要求与验收口径。
...
### F. 前端先抽状态，再抽视图（PCAP 工作台 2026-09-20 已完成）
...
落地位置（PCAP 工作台）：状态与 API 编排收拢到
`modules/network/pcap/composables/usePcapWorkbench.ts`（382 行），`PcapWorkbench.vue` 只保留
模板、弹窗与格式化（582 → 312 行）。与上述计划的两处差异：一是按现有耦合只抽一个 composable
而不是四个（列表/上传/轮询/分页/文件预览互相调用，拆四个会立刻互相 import）；二是模板里
`@closed="++fileVersion"` 改为调用 `closeFileDialog()`——`let` 计数器无法通过返回值暴露给模板，
返回值只是取值拷贝，直接自增会丢作废语义。被移动的 278 行脚本除这两处外逐行未改。
新增 `src/__tests__/pcap-workbench-state.test.ts`（9 项：列表失败、两次打开竞态、去重上传定位、
空文件拒绝、包分页与过期页丢弃、流跟踪与文件预览、关弹窗后丢弃迟到预览、轮询成功后重开抓包、
返回列表）；组件行为仍由 `pcap-workbench.test.ts`（3 项）覆盖。
落地位置（数据资产采集任务页）：状态与 API 编排收拢到
`modules/data-security/composables/useDataAssetJobs.ts`（159 行），`DataAssetJobs.vue` 只保留模板与
按钮（251 → 131 行）；探针与扫描配置下拉、派发/取消/移除、5 秒自动刷新 timer 都在 composable 里，
卸载时清除 timer。与计划的差异只有一处：页面原来直接调 `apiGet('/tasks', ...)`，改用已有的
`api/tasks.ts::listTasks`（同一个请求）；被移动的 113 行脚本除这一处外逐行未改。
新增 `src/__tests__/data-asset-jobs-state.test.ts`（9 项：加载与失败、统计卡、路径优先 payload、
无探针拒绝派发、派发失败、取消/移除、5 秒轮询与关闭自动刷新、卸载停止轮询）。

落地位置（数据目录类型页）：数据类型中心与类型详情的状态收拢到
`modules/data-security/composables/useDataTypeCenter.ts`（64 行）与 `useDataTypeDetail.ts`（68 行），
两个页面只保留模板（154 → 117、134 → 99 行）；被移动的 37 行与 35 行脚本逐行未改。与计划的差异：
类型详情把路由 `category` 作为 `ComputedRef` 参数传入（composable 不再自己 `useRoute`），并新增
`setPage()`，模板分页由 `@current-change="(value) => (page = value)"` 改为 `@current-change="setPage"`。
新增 `src/__tests__/data-type-catalog-state.test.ts`（7 项：加载与服务端 totals、过滤不改总数、
探针范围标签与失败保留旧数据、按路由 category 加载、翻页/换类型重新查询、身份徽标四态、失败进页面状态）。

落地位置（数据目录对象/实例页）：数据对象详情与实例详情的状态收拢到
`modules/data-security/composables/useDataObjectDetail.ts`（104 行）与
`useAssetInstanceDetail.ts`（72 行），两个页面只保留模板、行跳转与时间格式化（219 → 159、193 → 153 行）；
被移动的 61 行与 40 行脚本逐行未改。与计划的差异：路由 id 以 `ComputedRef` 参数传入
（composable 不再自己 `useRoute`），对象页新增 `setDetectionPage()`，模板分页由
`@current-change="(value) => { detectionPage = value; loadDetections() }"` 改为 `@current-change="setDetectionPage"`。
新增 `src/__tests__/data-object-instance-state.test.ts`（8 项：按路由 id 加载对象与第一页检测、
翻页只重查检测、身份文案四态、证据抽屉成功与失败、实例历史开关与重查、失败进页面状态）。

落地位置（扫描配置/规则版本页）：扫描配置与规则版本的状态收拢到
`modules/data-security/composables/useScanProfiles.ts`（188 行）与 `useRuleVersions.ts`（141 行），
两个页面只保留模板（289 → 155、256 → 156 行）；去掉缩进后，被移动的 132 行与 96 行脚本逐行未改。
与计划的差异：扫描配置页新增 `setPage()`，模板分页由
`@current-change="(value: number) => { page = value; load() }"` 改为 `@current-change="setPage"`；
规则版本页模板逐字节未改。
新增 `src/__tests__/scan-profile-rule-version-state.test.ts`（17 项：扫描配置的加载/计数/翻页/草稿/保存/
删除确认/下发与拒绝、规则版本的加载/清空/探针分类/发布/回滚，两侧的失败进页面状态）。

落地位置（文件分析/网络 DLP/敏感发现页）：这三个页面的状态收拢到
`modules/data-security/composables/useFileAnalysis.ts`（110 行）、`useNetworkDlp.ts`（106 行）与
`useSensitiveDiscovery.ts`（63 行），页面只保留模板（185 → 115、142 → 83、102 → 71 行）；
去掉缩进后，被移动的 67 行、65 行与 36 行脚本逐行未改，三个页面模板逐字节未改，本批没有模板改动。
与计划一致的部分：4 秒抽屉刷新 timer 归 composable（卸载即停，抽屉关闭时不轮询），
网络 DLP 的 `evidenceRequest` 竞态守卫随 `openTransfer()` 一起进 composable；唯一修饰改动是
`FileRecord`/`FileDetail` 由 composable 导出、视图按类型导入。
新增 `src/__tests__/file-analysis-state.test.ts`（9 项）与 `network-dlp-discovery-state.test.ts`（13 项）。

落地位置（事件中心/告警中心页）：两个页面的状态收拢到
...
两个页面模板逐字节未改，本批没有模板改动。与计划的差异：没有——筛选字段配置与攻击阶段标签
按「静态展示留在视图」的规则留在页面，投影（`activeStages`/`findings`/`confidence`）与全部
API 调用进 composable。
新增 `src/__tests__/incident-alert-center-state.test.ts`（17 项）。

落地位置（资产中心页）：状态收拢到 `modules/asset/composables/useAssetCenter.ts`（171 行），
页面只保留模板（290 → 147 行）；去掉缩进后，迁入的 145 行脚本逐行未改，模板逐字节未改，
本批没有模板改动。与计划的差异：删除了一处从未使用的 `useRouter()`（模板与脚本都没有引用它），
其余为机械搬迁；筛选字段配置与格式化函数按「静态展示留在视图」的规则留在页面。
新增 `src/__tests__/asset-center-state.test.ts`（15 项）。

落地位置（检测中心页）：状态收拢到
`modules/operations/detections/composables/useDetectionCenter.ts`（101 行），页面只保留模板
（183 → 111 行）；去掉缩进后，迁入的 78 行脚本逐行未改，模板逐字节未改，本批没有模板改动。
与计划的差异：没有——筛选字段配置留在视图（引擎下拉选项由 composable 暴露的 `engineOptions`
提供），列表、详情与手动流水线整块进 composable；composable 的导入路径是 `../../../../api/...`
（比数据安全域深一层）。
新增 `src/__tests__/detection-center-state.test.ts`（10 项）。

落地位置（引擎详情页）：状态收拢到 `modules/engines/composables/useEngineDetail.ts`（114 行），
页面只保留模板（208 → 126 行）；去掉缩进后，迁入的 83 行脚本逐行未改，模板与样式逐字节未改。
与计划的差异：composable 接收路由派生的 `name`（`ComputedRef<string>`），路由、导航与静态标签
留在页面，与对象/实例详情页同一约定；另外删除了视图里因搬迁而不再使用的 `EngineStatus` 类型导入。
新增 `src/__tests__/engine-detail-state.test.ts`（11 项）。

落地位置（看板页）：状态收拢到 `modules/dashboard/composables/useDashboard.ts`（94 行），页面只保留
模板（195 → 127 行）；去掉缩进后，迁入的 69 行脚本逐行未改，模板与样式逐字节未改。与计划的差异：
视图保留路由与 `formatDateTime`/`formatRiskScore`，以及模板里 `Incident`/`Asset` 类型注解需要的类型
导入；另外删除了视图里因搬迁而不再使用的 `utils/mapping` 导入（`levelBreakdown` 与三个映射表一起
进了 composable）。
新增 `src/__tests__/dashboard-state.test.ts`（7 项）。

落地位置（安全审计页）：状态收拢到
`modules/operations/audit/composables/useSecurityAudit.ts`（69 行），页面只保留模板（167 → 121 行）；
去掉缩进后，迁入的 50 行脚本逐行未改，模板与样式逐字节未改。与计划的差异：没有——静态标签
（`riskLabels`）留在视图，服务端数据的投影（`matchGroups`）随状态进 composable；它由模板直接调用，
因此 composable 把它一并导出。
新增 `src/__tests__/security-audit-state.test.ts`（7 项）。

落地位置（流量视图页）：状态收拢到
`modules/network/traffic/composables/useLiveTraffic.ts`（77 行），页面只保留模板（143 → 90 行）；
去掉缩进后，迁入的 53 行脚本逐行未改，模板与样式逐字节未改。与计划的差异：没有——路由与格式化
函数留在视图；长连接（`EventSource`）按「timer/socket 归 composable 所有并在卸载时清除」的规则
一起进 composable。
新增 `src/__tests__/live-traffic-state.test.ts`（8 项）。

落地位置（算法评估页）：状态收拢到
`modules/tools/composables/useAlgorithmEvaluation.ts`（141 行），页面只保留模板（306 → 196 行）；
去掉缩进后，迁入的 116 行脚本逐行未改，模板与样式逐字节未改（175 行）。与计划的差异：没有——
两个标签页的状态一起进 composable；静态语言下拉与三个展示组件留在视图；纯本地的密码评估与复杂度
分析不产生额外请求，只读探针列表与单个探针的密码画像。
新增 `src/__tests__/algorithm-evaluation-state.test.ts`（15 项）。

落地位置（威胁情报与规则三页）：状态收拢到 `modules/threat/composables/useRulesCenter.ts`（74 行）、
`useCveCenter.ts`（92 行）与 `useIocCenter.ts`（59 行），三页只保留模板（110 → 65、126 → 60、
118 → 82 行）；去掉缩进后，迁入的 52/70/39 行脚本逐行未改，模板与样式逐字节未改。与计划的差异：
没有——静态展示（执行状态标签、CVSS 等级映射、筛选字段配置）留在视图；轮询（Grype 任务 2s）按
「timer 归 composable 所有并在卸载时清除」的规则一起进 composable。
新增 `src/__tests__/threat-center-state.test.ts`（32 项）。
...
### I. 规则源与网络 DLP 分层（2026-09-20 已完成）

落地位置：网络 DLP 拆成 `app/services/dlp/{constants,policy,self_traffic,capture,detect}`，
`services/dlp_service.py` 只保留兼容重导出；`masked()` 移到中立的 `services/masking.py`，打断
「规则库 ↔ 网络 DLP」双向导入；规则库布局收敛到 `rule_library.rule_store_directory()` /
`rule_store_files()` / `set_rule_enabled()`；规则库到引擎与规则包的映射收敛到
`rule_library.stored_rule()`，由 `sensitive_engine.analyst_rules()` 与
`ruleset_service.import_working_rules()` 共用。边界由 `tests/test_dlp_boundaries.py`（7 项）锁定。
下列条目保留为当时的实施要求与验收口径。

1. 只移动文件位置与依赖方向：不改 API 路径、方法、响应结构，不改规则库文件格式与规则包格式，
   不改探针协议；旧入口 `services/dlp_service` 保留到调用方迁完。
2. monkeypatch 目标（`reassemble` / `http_objects`）迁到真实查找位置 `app.services.dlp.capture`，
   不为了迁就旧测试在实现层留转发。
3. 命中原文口径不变：只走 `matches`（每条命中 ≤3 条、`value` ≤120、`context` ≤240），其余字段仍不含值；
   关键词与指纹命中的 `rule_source` 不变，类别过滤与「手写规则不过滤」不变。
4. 不改告警阈值与曝光度：`min_confidence`、`alert_policy.high_finding_min_risk` 与
   `_capture_exposure()` 原样保留。

验收：`tests/test_dlp_boundaries.py`、`test_dlp_detection_quality.py`、`test_rule_libraries.py`、
`tests/shared/test_engine_wiring.py`、`test_bugfix_regressions.py` 全绿；后端全量与基线逐项一致
（6 项失败不变），新增用例只增不减。
...
| 前端 | `frontend/src/__tests__/pcap-workbench.test.ts`、`pcap-workbench-state.test.ts`、typecheck、全量 vitest |
...
## 第三十一批落地：数据库连接状态与视图（2026-09-21，步骤 F 续批）

原 `useDatabaseConnections.ts`（331 行）同时维护列表、编辑、库表范围、派发、详情和 timer；现保留
120 行协调入口，拆为 `useDatabaseConnectionForm`（93）、`useDatabaseScope`（86）与
`useDatabaseScans`（59）。依赖通过 Ref/ComputedRef 与回调传入，子模块不反向 import 协调入口。
`DatabaseConnections.vue`（323 → 186）抽出表单（84 行）与详情抽屉（83 行），API 路径、参数和行为不变。

验收：修改前 typecheck / 15 项定向通过；修改后 typecheck / 22 项定向 / 31 文件 277 项全量通过；
已对照本轮快照检查函数体与还原模板。新增 7 项行为回归覆盖跨模块接线、错误保留和卸载清 timer，
没有添加仅断言文件存在的测试。未提交、未重建镜像或离线包、未操作真实目标；生产构建结果见 TASK.md。
