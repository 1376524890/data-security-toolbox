...
**① 自动刷新：把三件事收进一个 composable**（`frontend/src/composables/useAutoRefresh.ts`）。
原先每个列表页各写一份 `setInterval`，于是各错各的。现在统一三条保证：**上一拍没回来不再排下一拍**
（慢接口不会瞬间变成一串并发请求）、**`document.hidden` 时完全不轮询**（被最小化的大屏没人看，
后台轮询只烧数据分区）、**卸载清 timer**（活过组件的 timer 会往已销毁的 ref 里写）。
刷新一律 `load({ silent: true })`：**后台失败保留上一次成功的数据**，不把列表清空成假象。
间隔按数据变化速度给：任务中心 15 s、抓包工作台/看板 30–60 s、规则库/漏洞库 120 s；
`LoginView`、编辑中的表单不轮询。

**② 筛选 + 点击列标题排序**。前端 `useTableSort` 把 Element Plus 的 `@sort-change` 归一成列表 API
只认的 `order_by` 拼写（`field` 升序 / `-field` 降序），并**同时把页码复位到第 1 页**——只换顺序不换页码，
看到的是新顺序下早已不存在的那一页。后端新增 `app/api/list_sort.py`：白名单之外**直接 400**
（`{"error":"unsupported_sort","allowed":[...]}`）而不是静默忽略，静默忽略会让页面顶着一个从未生效的
排序箭头；白名单同时是安全边界，查询参数永远变不成未建索引的全表扫描。已接入 `/tasks`、`/files`、
`/pcaps`、`/data/assets`、`/network/assets`、`/asset-instances`（并新增 `severity` 过滤）、`/offline/cves`
（并新增 `severity`/`source` 过滤）；敏感度按 `SEVERITY_RANK` **按等级**排序而不是按字母拼写。
自己不翻页的列表（规则库、策略组、指纹候选、评估发现、传输对象、规则版本）在浏览器内筛选与排序，
不为此多打一次请求；规则版本页的两张表**各自持有排序状态**——共用一份会让点探针表的表头把版本表
也一起重排。

**排序期间才现形的一个缺陷**：`order_rows()` 用 `sorted(..., reverse=True)`，**空值被 reverse 顶到最前面**
——`None` 读成「最高」而不是「未知」。现在先把有值的行排序、缺值的行原样接在后面，两个方向都成立。
这个缺陷是先写了「空值恒在最后」的断言才暴露出来的。

**③ OCR 补齐数据安全扫描的盲区**。平台此前只看得到**有文本层**的文件：扫描版 PDF 与图片一律落成
`binary_metadata_only`——**登记在册、从未被检查**。现在图片与扫描版 PDF 经 `tesseract`（`chi_sim+eng`）
+ `pdftoppm` 读出文字（`shared/scanning/ocr.py`），**边界写死**：单文件最多 20 页 / 20 万字符、单页 60 秒
超时、单图 30 MB 上限、版式分析在 256 px 缩略图上做（逐像素循环不碰原图），每一字节都记进共用的
`ScanBudget`。**覆盖率如实上报**（`complete`/`partial`/`failed`/`unavailable`/`unsupported`）：
缺工具链就是「未检查」，绝不装成「干净」；解析出文字但被字符上限截断也算 `partial`。
工具链加进 `backend/Dockerfile` 的 base 阶段，api 与 analysis-worker 两个镜像共用。

**红头 / 涉密判定**（`shared/scanning/document_types.py`）：把 OCR 文本与页面颜色变成**可解释的信号**，
而不是一个分数。涉密标志要求 `密级★期限`、`密级：X` 的形状，或两个以上独立涉密用语；**光一个「秘密」
不算**（「秘密地点」也会命中）。红头文件要求**版式红头与公文文字互证**（发文字号、以「文件」结尾的标题行、
`关于……的通知`、发文机关、成文日期）；只有红头没有公文文字时只报「红头版式文件（Low）」，
不替对方拔高。产出两条规则：`DATA_CLASSIFIED_001`（绝密/机密 Critical、秘密/通用 High）与
`DATA_REDHEAD_001`（Medium）。**文件源扫描**（`services/file_scan/ingest.py`）同样接上：OCR 文本进入
同一套敏感扫描，覆盖率由 `binary_metadata_only` 变成 `partial / ocr_read`，`ocr` 与 `document_signals`
随资产 `extra` 落库。探针**不做 OCR**——依赖与负载留在服务端，`shared` 里的实现无工具链时自动降级。

**⑤ 密码评估页按资产常驻展示结果**（用户本轮追加的要求）。原页面列的是**检查任务**，点一条才在下面
渲染那次运行观测到的主机——结果被绑在「哪次跑的」上，而运维问的是「这台资产现在怎么样」。现在
`composables/useCryptoAssessmentResults.ts` 把当前页每条含密码评估的检查任务**折叠成一行一台主机**
（`toAssetRows()`），同一主机被多次观测时**取最新一次**（`finished_at` 再按 id），行里直接带结论
`level`/`overallScore`/`violations`，并按最近观测倒序；检查任务表降级为**观测来源**。
判定口径只有一份：把 `useCryptoAssessment.ts` 里「观测值优先、缺项用内置默认补齐」的规则抽成导出的
`configFromProfile()`，页面与资产行都走它再调 `assessCrypto()`——两份实现会让同一台主机在两个页面
评出不同等级。资产表支持筛选与点击列标题排序，**等级按结论档位排序**（不是 `合规/不合规` 的字符串
顺序）；两张表**各持一份排序状态**，共用一份会让点一张表把另一张也重排。面板只有一个：点资产看常驻
结论、点任务看那次运行的主机，两边互斥，刷新后选择项不在当前页时回落到资产视图。**无后端改动、无迁移**。

**④ 更名**：用户可见的产品名统一为「数据安全监测检测工具箱」（后端 `settings.app_name`、报告模板、
通知文案、控制台标题/登录页/导航、种子脚本、部署脚本、README，以及探针 `--help` 的守护进程描述）。
**刻意不动**标识符与安装路径：systemd 单元 `data-security-toolbox-probe`、`/opt/data-security-toolbox`、
`/etc/data-security-toolbox`、`/var/lib/data-security-toolbox` 保持原样——改名换来的是已下发探针失联，
不是收益。

**测试与交付**：前端 **23 文件 / 147 项通过**（原 22 / 124，新增 `auto-refresh-table-sort.test.ts` 15 项，
`rule-versions-state.test.ts` 补 4 项，`crypto-assessment-results-state.test.ts` 补 6 项），
`vue-tsc --noEmit` 无输出；
后端全量（容器内，带 `--ignore=tests/test_rule_libraries.py`）
**35 failed / 1000 passed / 5 skipped / 8 errors**。为了不靠「和上次的数字比」，做了**同布局 A/B**：
同一个 `worktree` 布局下把工作区换回 HEAD 跑一次（**44 failed / 957 passed / 5 skipped / 8 errors**），
两次失败集合 **diff 为空**（基线多出的 9 项全部是需要 `probe_packages/` 的分发包测试，干净 worktree 里
本来就没有该目录），通过数 957 → **1000** = 新增 34 项 + 转绿 9 项。ruff 对新增文件零告警；
被改文件的告警集合与 HEAD 相同或更少（`engine.py` 12 E501 不变、`ingest.py` 8 → 7、
`offline_manager.py` 29 → 27、`integrations.py` 0 → 0）。
...
每 2 秒轮询一次，`job_path` 找不到文件就 404，`poll()` 把 404 当普通错误写进 `error` **并继续排下一次**——
于是既清不掉报错也停不下来。现在 `api/client.ts` 把 HTTP 状态挂在 Error 上，composable 认 404 为
「记录已不存在」：清 localStorage、复位 busy、提示一次「已清除本地记录」并**结束轮询**；非 404 仍旧重试。
**判别依据是状态码而不是文案**，所以服务端换措辞不会让这条逻辑失效。

**② 抓包段刷屏任务监控**：`default_task_filter()` 原先把「是不是子任务」完全押在
`payload['monitor_task_id']` 上，而挂链只发生在上传那一刻——上传时没有 Running 监控任务、或挂链机制
存在之前建的行，都会留下**没有链接的段**，它们满足过滤条件，于是一条条裸 `pcap` 行挤满任务中心
（本机 76 行，`/tasks` 最新 8 行全是 `pcap`）。现在默认列表**直接排除 `kind='pcap'`**（用
`monitoring.SEGMENT_KIND`），不再依赖那个字段；段仍可按 `kind=pcap` 检索（任务中心类型下拉里本来就有）。
**过滤即修复，没有改一行历史数据**。

**③ 大屏两张图分开 + 地图上画出数据链路**：中心列原来是 `TrafficMap` 与 `GeoMap` 上下并列各占一半，
现在改成 **`数据流动` / `地理位置态势` 两个可切换视图**（同一时刻只渲染一张，当前图拿满整列高度），
切换状态 `centerView` 与 `metric` 一样归 composable。地理态势里**画出数据流动链路**：内网 hub → 每个
目的国家/地区，粗细=数据量、颜色=该组敏感等级、虚线沿方向流动，悬停给出「内网 → 美国 · N 主机 · 流量 ·
等级」；链路按**服务端能给出的最小真实粒度**（目的地区分组）绘制，**不画主机到主机**——接口只把目的地址
...
**真机部署验证（192.168.110.50，arm64 / 银河麒麟）**：交付包整机替换了 `.50` 的原部署
（旧目录保留为 `dst-toolbox-3.2.0-linux-arm64.prev-<时间戳>`，可回滚）。9 个容器全 Up 且健康、
`/openapi.json` 为 **3.2.0**、控制台 `/` 与 `/cockpit` 均 200、端口收敛规则仍在
（仅 `192.168.110.0/24` 可达 8080/8000）。探针下发到 `192.168.110.168`：preflight 全 PASS、
`ONLINE`，真实命令行带 `-f not (host 192.168.110.50 and port 8000)`，抓包段由 64 MB 降到
**1.9 MB / 0.15 MB**，段内指向 `192.168.110.50:8000` 的流 **0 条**，3 段全部 `analyzed`；
任务监控默认列表只剩 1 条 `monitoring`，无 `pcap` 刷屏。
...
（1.28 GB，sha256 `a7ba4865…b8492c`，包内 6 个 arm64 镜像 / 不含 `.env`）。
...
状态 `Partial`、终止原因 `UnicodeDecodeError`、`complete_scope=false`。

**根因（已实测复现）**：目标机 `/home/gb` 里有一个**文件名不是合法 UTF-8** 的条目（字节 `0x9d`）。
paramiko 的 SFTP 对文件名做严格 UTF-8 解码，列该目录时直接抛 `UnicodeDecodeError`；而
`services/file_scan/scan.py` 的异常处理是**整轮扫描级**的——一个目录列不出来就跳出整个 `while`，
该目录之后的内容全部没扫，结果却像一次正常结束的扫描（最危险的地方：看起来"干净"）。

**修复**：① 目录级与文件级异常各自收口，坏目录记为 `unreadable` 并**继续**，坏文件以
`coverage=failed` 落库而不是丢弃；② SFTP 列目录失败时回退到 `ls -1a`（exec 通道）按字节自行解码，
解不出来的名字记为 `skip` 而不是整目录放弃；③ 任务结果新增 `unreadable`/`unreadable_count`，
`source.last_error` 也会写明"N 个目录/文件未能读取"；④ 向导的目录树浏览同样把坏目录标为不可读节点。

**复验（真实目标）**：同一来源重跑，从 57 个文件提升到 **200 个文件**（`assets=200`、`sensitive=39`、
`skipped=12`、`unreadable=0`），终止原因变成 `file_budget`（正常的文件预算上限），坏目录已可正常读取。

同时前端两处交互改进：向导的检测范围改为**可展开的懒加载目录树 + 级联勾选**（勾父目录即选整棵子树，
文件只展示不可勾），以及把可选择的填空改成备选项（用户名、根目录、远程协议、定时间隔、置信度、命中次数、
规则类别）。

2026-09-21 第三十四批（已推送：`140066b` → `8080857`）：**六个板块的完整重构**。

> **设计冻结 + 验证状态（重要）**：这六个板块（资产中心 / 任务中心 / 数据资产 / 数据流动与防护 / 文件证据 /
> 采集与规则）及其子项**自本条起冻结**，`frontend/src/router/menu.ts` 顶部已写明本次冻结；再改就是设计变更而非
> 重构。**功能尚未验证正常运行**：只做了结构性验证（`vue-tsc`、`vitest`、镜像构建、`/api/v1/health` 与
> `:8080` 的 HTTP 冒烟），**没有做端到端验收**——没有浏览器点选走查、没有真机探针下发、没有真实扫描/抓包，
> 且向导的"下发探针"链路依赖本机未配置的 `DEPLOYMENT_BACKEND_URL`。在完成一次真实走查前，请一律按"未验证"对待。

控制台侧边栏收敛为 **6 个大板块**（资产中心 / 任务中心 / 数据资产 / 数据流动与防护 / 文件证据 / 采集与规则），
每个板块下的页面按数据安全主线重建，删掉的页面连同其前端与后端死代码一并清理。

- **任务中心**：单页 = 任务列表（进度/健康/可展开详情与简要问题报告）+ 右上角「新建任务」分步向导
  （目标主机 IP/端口 → 连接方式与凭据〔密码/私钥〕→ 连通性测试 → **同步枚举目录树并勾选范围** → 检测项/单次或定时）。
  新增 `POST /api/v1/targets/test`（一次 SSH 连接即关，凭据不落库不回显）与 `POST /api/v1/targets/browse`
  （深度/条目/墙钟三重上限，超限如实 `truncated`）。无探针目标复用 SFTP 只读链路；探针是**任务专属临时探针**：
  凭据加密保留至任务终态，任务结束后按引用计数自动卸载并销毁凭据（手工注册的探针永不回收）。
- **资产中心**：只保留一个数据大屏，并入 `/assessments/overview` 的数据安全概览条；删除探针管理/探针部署/健康状态/
  报告中心/安全审计/资产大屏。
- **数据资产**：合并为单页（概览与分级分布 + 风险/脆弱数据清单与评级 + 风险分布），数据全部来自只读评估聚合。
- **数据流动与防护**：只保留**风险数据流动报告**与**数据出境报告**，页面内无任何规则配置，只有规则集开关；
  新增离线 **CIDR→地区表**（RIR delegated 生成，239 国 / 262,935 条 / 4.7 MB，二分查找），出境判定不再只能报
  「无法判定」；修掉黑白名单命中判断依赖地区标签的真 bug。
- **文件证据**：风险文件 + 风险 PCAP（复用现有 PCAP 解析与展示组件）。
- **采集与规则**：只保留 规则集与规则（敏感数据发现 + 网络流量监控的正则）、规则版本、策略分组、出境判定名单。
- **验证**：前端 `vue-tsc` 通过、`vitest` 13 文件 / 60 项通过；后端 targets/egress/探针回收/策略组/评估/边界共
  数十项通过（仅剩容器无 Redis 的既有 health 失败）。镜像已重建并切换，`/api/v1/health` 200、控制台 200。
